...
- 头条可能从正文插图自动同步封面。若点击封面槽后没有出现“本地上传”面板，服务会复核目标 `.article-cover-img-wrap`；槽位已经存在真实图片时直接采用平台同步结果，不再误报上传失败。预览按钮和空槽加号图标不会被当作有效封面。
...
- 图文标题最多 30 个头条加权字符：ASCII 英文字母和数字按 0.5 计，空白不计，中文及标点等其他字符按 1 计。标题超限时服务会直接返回可读错误，要求调用方重新生成或缩短标题，不会自动截断破坏语义。
- 发布过程中若正文、插图、封面或最终提交失败，服务会保留头条后台可能已经自动保存的草稿，并在错误信息中提示前往草稿箱检查。
- 普通发布失败不会自动调用删除接口。只有用户显式调用删除功能，或集成测试清理其自身创建的临时数据时，才会删除草稿。
- 点击“预览并发布”进入预览页不代表发布完成；服务会继续点击预览页中的“发布/确认发布”，仅在跳转到明确管理页或出现成功提示后返回成功。
