...
8. **React 受控状态固化与时序防线**：
   ...
   - **预览页不是发布成功页**：点击“预览并发布”后离开 `/graphic/publish` 可能只是进入预览/确认页面。不得用“URL 不再包含 publish”作为成功条件；必须继续检测并点击预览页中的“发布/确认发布”，且只有进入明确的管理页或出现成功提示后才能判定完成。
...
20. **标题完整性与失败草稿保护（硬性内容安全规则）**：
    - 图文标题长度必须按头条权重计算：ASCII 英文字母和数字计 `0.5`，空白不计，中文及标点等其他字符计 `1`。加权长度超过 `configs.MaxTitleLength`（当前为 30）时，MCP 入口、服务层和浏览器输入层都必须返回明确校验错误，提示调用方重新生成或缩短标题。严禁使用 `utf8.RuneCountInString` 直接判定标题上限，也严禁通过切片、截断或省略号自动改写用户标题。
